...
public class QuestionController {
    ...
    public ResponseEntity<List<Reponse>> getOptions(@PathVariable long questionId) {
        ...
        long userId = appUserRepository.getByEmail(SecurityContextHolder.getContext().getAuthentication().getName())
                .getId();
        PasseExamen passage = passeExamenRepository.findByAppUserIdAndExamenId(userId, question.getExamen().getId())
                .orElseThrow();
        if (passage.getEtat() != PasseExamenEtat.DEMARRE || !passeExamenRepository
                .findByAppUserIdAndExamenId(userId, question.getExamen().getId()).isPresent()) {
